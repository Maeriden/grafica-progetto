 ...
 vec3 phong(vec3 point, vec3 normal, vec2 uv,
            vec3 light_pos, float light_power)
 {
 	...
 	vec3  V = normalize(light_pos - point);          // Direction from the contact point to the camera
 	vec3  L = normalize(light_pos - point);          // Direction from the contact point to the light
 	...
 	{
 		float NdotL   = dot(N, L);
 		color_diffuse = light_color * max(0.5f, I) * max(0, NdotL);
 		
 		// Sun lighting formula
 		// color_diffuse  = vec3(max(0.1, NdotL+0.5));
 	}
 ...
 bool ray_cast(inout vec3 ray_pos, const vec3 ray_dir, inout float total_distance)
 {
	...
	bool  did_hit  = false;
 	...
 	for(int i = 0; total_distance <= MAX_DISTANCE; ++i)
 	{
		...
		if(closest_distance < raymarch.epsilon)
		{
			if(total_distance >= camera.planes[0])
			{
				did_hit = true;
			}
			break;
 ...
 void main()
 {
 	...
 	vec3 background = vec3(0.185, 0.19, 0.2) * (2*ray_dir.y + 2);
 	...
 	if(did_hit)
 	{
		...
		// result.rgb          = vec3(color, color, color);
 		...
 		vec3  sun_dir = normalize(vec3(0, 1, -0.5));
 		vec3  sun_pos = 100.0f * sun_dir; // Make sun position relative to point to simulate infinitely far light source
 		float sun_pow = 10000.0f;
 		
 		vec3 color = phong(point, normal, uv, sun_pos, sun_pow);
